...
pblast.payloadSize = 1000;
pblast.l3protocol = TCP;
pblast.l2protocol = IPV4;
pblast.l1protocol = ETHERNET;
pblast.streams = 1;
pblast.flows = 100;
    ...
    while (1) {
        ...
        for (flows=0;flows<pblast.flows;flows++)
        {
            ...
            if (likely(pblast.l3protocol == TCP)) {
                ...
                udp_hdr->src_port = htons(900);
                udp_hdr->dst_port = htons(80);
                //udp_hdr->dgram_len = ;
                //udp_hdr->dgram_cksum = ;
            } else if (likely(pblast.l3protocol == ICMP)) {
                icmp_hdr = rte_pktmbuf_mtod(m, struct icmp_hdr *);
                //icmp_hdr->icmp_type;
                //icmp_hdr->icmp_code;
                //icmp_hdr->icmp_cksum;
                //icmp_hdr->icmp_ident;
                //icmp_hdr->icmp_seq_nb;
            } else if (likely(pblast.l3protocol == IGMP)) {
...
//                    pkt_size += sizeof(struct tcp_hdr);
                ...
                ipv6_hdr = (struct ipv6_hdr *) rte_pktmbuf_prepend(m, (uint16_t)sizeof(struct ipv6_hdr));
                //ipv6_hdr->vtc_flow;
                //ipv6_hdr->payload_len;
                //ipv6_hdr->proto;
                //ipv6_hdr->hop_limits;
                //ipv6_hdr->src_addr[16];
                //ipv6_hdr->dst_addr[16];
            } else if (likely(pblast.l2protocol == ARP)) {
                //arp_hdr = (struct arp_hdr *) rte_pktmbuf_prepend(m, (uint16_t)sizeof(struct arp_hdr));
                //arp_hdr->arp_hrd;
                //arp_hdr->arp_pro;
                //arp_hdr->arp_hln;
                //arp_hdr->arp_pln;
...
//printf("L3 Protocol  %d\n",pblast.l3protocol);
//printf("L2 Protocol  %d\n",pblast.l2protocol);
//printf("L1 Protocol  %d\n",pblast.l1protocol);
//printf("Streams  %d\n",pblast.streams);
//printf("Flows %d\n",pblast.flows);
